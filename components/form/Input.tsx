...
const Input: FC<InputPropsI> = ({
	...
	return (
		<div className="pb-6 last:pb-0">
			<label
				htmlFor={name}
				className={`w-full border-2 rounded-xl text-gray-40 flex gap-2 mb-2 ${touched && error ? 'border-red-500' : 'border-gray-300'} ${classNamesLabel}`}
			>
				{icon && <span className="text-gray-40">{icon}</span>}
				<input
					style={{ textAlign: "inherit" }}
					className={`flex-1 w-full outline-none rounded-[9px] text-black ${classNamesInput}`}
					name={name}
					id={name}
					{...props}
				/>
